...
| `tailwindcss` 3 → 4 | new oxide engine, CSS-first config, breaking config format | High | ✅ done (4.2.4). Used official `@tailwindcss/upgrade` tool: migrated `tailwind.config.js` to CSS-first `@theme` block in index.css, swapped `tailwindcss` PostCSS plugin for `@tailwindcss/postcss`, removed `autoprefixer` (built in), and renamed v4-changed classes across 20 files (e.g. `shadow-sm`→`shadow-xs`, `backdrop-blur-sm`→`backdrop-blur-xs`, `outline-none`→`outline-hidden`, `rounded-sm`→`rounded-xs`). Compat shim added for default border color change |
